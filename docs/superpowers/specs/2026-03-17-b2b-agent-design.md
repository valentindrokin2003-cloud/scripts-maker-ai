...
| `revenue_max` | "Объем выручки" (верхняя граница) | int\|null | `null` (не применяется) |
...
### 10. `##AGENT:revenue##`
Агент генерирует два варианта в зависимости от наличия верхней границы.

**Только нижняя граница** (например, "От 1 миллиона рублей" — `revenue_max = null`):
```python
# ##AGENT:revenue##
start_rev = 1_000_000
...
**Диапазон** (указаны обе границы — `revenue_max` задан):
```python
# ##AGENT:revenue##
...
| `revenue_max` не указан | Верхняя граница не применяется, `end_rev` не генерируется |
